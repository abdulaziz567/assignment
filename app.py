...
def insert_violation(conn, plate_number, violation_type, confidence, image_name, frame_number=None):
    if conn is None:
        return False
    try:
        cur = conn.cursor()
...
def fetch_violations(conn):
    ...
    try:
        df = pd.read_sql(
            "SELECT * FROM violations ORDER BY timestamp DESC LIMIT 200", conn
        )
        return df
    except Exception:
        return pd.DataFrame()
...
# ─── Detector Setup ───────────────────────────────────────────────────────────
@st.cache_resource
def load_detector(conf_thresh, iou_thresh):
    """Load real YOLO detector. Returns None if model files not found."""
    if not DETECTOR_AVAILABLE:
        return None
    seatbelt_path = "models/seatbelt_yolov8.pt"
    plate_path    = "models/license_plate_yolov8.pt"
    if not (os.path.exists(seatbelt_path) and os.path.exists(plate_path)):
        return None
    try:
        return SeatbeltDetector(
            seatbelt_model_path=seatbelt_path,
...
def run_detection(image_np, detector, conf_thresh, iou_thresh):
    """
    Run real YOLO+OCR if detector available,
    otherwise fall back to mock (for demo only).
    """
    ...
    for i in range(random.randint(1, 3)):
        ...
        x2 = min(x1 + random.randint(80, 180), w - 1)
        y2 = min(y1 + random.randint(100, 220), h - 1)
...
def draw_detections(image_np, detections):
    """Draw bounding boxes on image (used when real annotate() not available)."""
    if DETECTOR_AVAILABLE:
        return annotate(image_np, detections)
    img = image_np.copy()
...
with st.sidebar:
    ...
    <div style='margin-bottom:24px'>
        ...
        <div style='font-size:0.7rem;color:#5a6075;letter-spacing:0.12em;text-transform:uppercase;
                    font-family:"Space Mono",monospace'>Violation Monitor v2.0</div>
    </div>
    """, unsafe_allow_html=True)
    ...
    conf_thresh = st.slider("Confidence Threshold", 0.3, 1.0, 0.60, 0.05)
    ...
    st.caption(
        "Detects seatbelt violations, extracts license plates via OCR, "
        "and logs all violations to PostgreSQL."
    )
...
<div style='height:1px;background:linear-gradient(90deg,#f5c518,transparent);margin-bottom:20px'></div>
""", unsafe_allow_html=True)

# ── Model status banner ───────────────────────────────────────────────────────
if detector is not None:
    st.markdown("""
    <div class="model-status-box" style="border-left:4px solid #00e5a0">
        ✅ &nbsp;<strong>Real YOLOv8 models loaded</strong> — Full YOLO + Tesseract OCR pipeline active.
    </div>""", unsafe_allow_html=True)
else:
    st.markdown("""
    <div class="model-status-box" style="border-left:4px solid #f5c518">
        ⚠️ &nbsp;<strong>Demo mode</strong> — Model weights not found
        (<code>models/seatbelt_yolov8.pt</code> / <code>models/license_plate_yolov8.pt</code>).
        Place trained weights in the <code>models/</code> folder to activate real detection.
        See <strong>README.md → Training</strong> for instructions.
    </div>""", unsafe_allow_html=True)
        ...
        if uploaded:
            ...
            if st.button("🔍  Run Detection", use_container_width=True):
                with st.spinner("Running YOLO + OCR pipeline..."):
                    ...
                    time.sleep(0.2); progress.progress(20, "Detecting persons…")
                    time.sleep(0.2); progress.progress(50, "Filtering violations…")
                    detections = run_detection(img_np, detector, conf_thresh, iou_thresh)
                    time.sleep(0.2); progress.progress(75, "Detecting license plates…")
                    time.sleep(0.2); progress.progress(90, "Applying OCR…")
                    annotated  = draw_detections(img_np, detections)
                    time.sleep(0.1); progress.progress(100, "Done ✓")
                ...
                for det in detections:
                    ...
                    if not det["has_belt"]:
                        ...
                        entry = {
                            ...
                            "plate":      det.get("plate") or "UNREADABLE",
                        ...
                        if conn:
                            insert_violation(
                                conn, entry["plate"], entry["violation"],
                ...
                if not det["has_belt"]:
                    ...
                    <div class="log-entry">
                        ...
                        <div style='margin-top:8px'>
                            Plate: <span class='plate-display'>{det.get('plate') or 'UNREADABLE'}</span>
                        </div>
                    </div>
                    """, unsafe_allow_html=True)
                else:
            ...
            if st.button("▶  Process Video", use_container_width=True):
                ...
                while cap.isOpened():
                    ...
                    if frame_num % sample_every == 0:
                        ...
                        dets = run_detection(rgb, detector, conf_thresh, iou_thresh)
                        annotated_frame = draw_detections(rgb, dets)
                        preview_slot.image(annotated_frame,
                    ...
                    status_text.caption(
                        ...
                        f"{processed} sampled · {video_viols} violations"
                    )
                    frame_num += 1
                ...
                os.unlink(tmp_path)
                status_text.success(
                    f"✓ Processed {processed} frames — {video_viols} violations logged"
                )
...
with tab4:
        ...
        if lc_video_file:
            ...
            if ret_p:
                ...
                if lc_axis == "horizontal":
                    lc_line_pos = st.slider("Line Position (Y pixel)",
                                            20, h_vid - 20, h_vid // 2, 2)
                else:
                    lc_line_pos = st.slider("Line Position (X pixel)",
                                            20, w_vid - 20, w_vid // 2, 2)
                ...
                st.image(preview_with_line,
                         caption=f"Line preview — {lc_axis} at "
                                 f"{'Y' if lc_axis=='horizontal' else 'X'}={lc_line_pos}",
                         use_container_width=True)

                st.markdown("<div style='height:8px'></div>", unsafe_allow_html=True)
                st.markdown('<div class="section-title">Processing Options</div>',
                            unsafe_allow_html=True)
                lc_sample_every = st.number_input("Process every N-th frame", 1, 30, 3,
                                                  key="lc_sample")
                lc_iou_thresh   = st.slider("Tracker IoU threshold", 0.15, 0.70, 0.30, 0.05,
                                            key="lc_iou")
                lc_max_missed   = st.number_input("Max missed frames before track drop",
                                                  2, 30, 8, key="lc_missed")

                btn_col1, btn_col2 = st.columns(2)
                with btn_col1:
                    run_lc   = st.button("▶  Start Analysis", use_container_width=True,
                                         key="run_lc")
                with btn_col2:
                    reset_lc = st.button("↺  Reset Tracker", use_container_width=True,
                                         key="reset_lc")
                ...
                if run_lc:
                    ...
                    while cap_lc.isOpened():
                        ...
                        if frame_num_lc % lc_sample_every == 0:
                            ...
                            for ev in events:
                                crossing_count += 1
                                crop_dets  = run_detection(ev.frame_crop, detector,
                                ...
                                if not has_belt:
                                    ...
                                    if conn:
                                        insert_violation(
                                            conn, entry["plate"], "No Seatbelt",
                                            confidence, entry["source"], frame_num_lc
                                        )
                                st.session_state.total_processed += 1

                            frame_vis = draw_virtual_line(
                                rgb_lc, lc_line_pos, lc_axis,
                                triggered=len(events) > 0, label=lc_label,
                            )
                            frame_vis = draw_vehicle_tracks(
                                frame_vis, tracker.tracks, events,
                                st.session_state.lc_results,
                            )
                            preview_lc.image(
                                frame_vis,
                                caption=f"Frame {frame_num_lc} | "
    ...
    with lc_col2:
        ...
        if st.session_state.lc_events_log:
            ...
            viol_ev  = sum(1 for e in st.session_state.lc_events_log
                           if "No Seatbelt" in e["violation"])
            ...
            for ev in st.session_state.lc_events_log[:15]:
                ...
                badge_html = ("<span class='violation-badge'>⚠ VIOLATION</span>"
                              if is_viol else
                              "<span class='safe-badge'>✓ COMPLIANT</span>")
                plate_html = (f"Plate: <span style='color:#f5c518;font-weight:700;"
                              f"letter-spacing:0.1em'>{ev['plate']}</span> &nbsp;|&nbsp; "
                              if is_viol else "")
                st.markdown(f"""
            ...
            st.download_button(
                "⬇  Export Crossing Events CSV", lc_csv,
